...
def extraer_features(ip):
    ...
    return [
        ...
        min(comp["comandos_ejecutados"], 10),
        variacion
    ]
...
def registrar_intento_ml(ip, username, password):
    ...
    # Re-entrenar cada 100 intentos
    total = sum(len(c["intentos"]) for c in _comportamiento.values())
    if total > 0 and total % 100 == 0:
        print(f"\n  🔄 {total} intentos acumulados — re-entrenando modelo ML...")
        threading.Thread(target=reentrenar_con_logs, daemon=True).start()
...
def reentrenar_con_logs():
    ...
    with open(ML_LOG, "r") as f:
        for line in f:
            ...
            try:
                ...
                fd = r.get("features", {})
                datos_reales.append([
                    fd.get("velocidad_intentos_min", 0),
                    fd.get("usuarios_distintos", 0),
                    fd.get("passwords_distintas", 0),
                    1 if fd.get("uso_diccionario") else 0,
                    1 if fd.get("interactuó_shell") else 0,
                    fd.get("comandos_ejecutados", 0),
                    fd.get("variacion_temporal", 0)
                ])
    ...
    print(f"  ✅ Modelo re-entrenado con {len(datos_reales)} ejemplos reales")
